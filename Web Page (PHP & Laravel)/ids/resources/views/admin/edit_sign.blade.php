...
<body>
  ...
  <section class="add_sign" id="add_sign">
    ...
    <form action="{{ route('signatures.update', $signature->id) }}" method="POST">  @csrf
      @method('PUT')  <div class="add_info mb-4">
        <div class="row">
          <div class="col-md-7 mb-4">
            <label for="action" class="form-label" style="color: white;">Action</label>
            <input type="text" class="form-control" id="action" name="action" value="{{ old('action') ?: $signature->action }}" aria-describedby="Action">
          </div>
          <!-- Protocol Field -->
          <div class="col-md-7 mb-4">
            <label for="protocol" class="form-label" style="color: white;">Protocol</label>
            <input type="text" class="form-control" id="protocol" name="protocol" value="{{ old('protocol') ?: $signature->protocol }}" aria-describedby="Protocol">
        </div>

        <!-- Source IP Field -->
        <div class="col-md-7 mb-4">
            <label for="source_ip" class="form-label" style="color: white;">Source IP</label>
            <input type="text" class="form-control" id="source_ip" name="source_ip" value="{{ old('source_ip') ?: $signature->source_ip }}" aria-describedby="Source IP">
        </div>

        <!-- Source Port Field -->
        <div class="col-md-7 mb-4">
            <label for="source_port" class="form-label" style="color: white;">Source port</label>
            <input type="text" class="form-control" id="source_port" name="source_port" value="{{ old('source_port')?: $signature->source_port }}" aria-describedby="Source port">
        </div>

        <!-- Destination IP Field -->
        <div class="col-md-7 mb-4">
            <label for="destination_ip" class="form-label" style="color: white;">Destination IP</label>
            <input type="text" class="form-control" id="destination_ip" name="destination_ip" value="{{ old('destination_ip')?: $signature->destination_ip }}" aria-describedby="Destination IP">
        </div>

        <!-- Destination Port Field -->
        <div class="col-md-7 mb-4">
            <label for="destination_port" class="form-label" style="color: white;">Destination Port</label>
            <input type="text" class="form-control" id="destination_port" name="destination_port" value="{{ old('destination_port') ?: $signature->destination_port }}" aria-describedby="Destination Port">
        </div>

        <!-- Options Field -->
        <div class="col-md-7 mb-4">
            <label for="options" class="form-label" style="color: white;">Options</label>
            <input type="text" class="form-control" id="options" name="options" value="{{ old('options')?: $signature->options }}" aria-describedby="Options">
        </div>
