      ...
      <div class="row">
        ...
        <!-- IP Info Display -->
          <div class="col-md-9 showw_info mb-3">
            <h3>ALERT Info</h3>
            <pre>{{ $alert->alert_info }}</pre>
          </div>
        
         <!-- IP Info Display -->
         <div class="col-md-9 showw_info mb-3">
          <h3>IP Info</h3>
          <pre>{{ $alert->ip_info }}</pre>
        </div>

        <!-- Transport Info Display -->
        <div class="col-md-9 showw_info mb-3">
          <h3>Transport Info</h3>
          <pre>{{ $alert->transport_info }}</pre>
        </div>

        <!-- Application Info Display -->
        <div class="col-md-9 showw_info mb-3">
          <h3>Application Info</h3>
          <pre>{{ $alert->application_info }}</pre>
        </div>
      </div>
    </div>
    <a href="{{ route('more_packets_log', ['alert_id' => $alert->packet_id]) }}" style="display: block; margin: 0 auto; text-align: center;">
      <button type="button" class="btn btn-primary">Show Network Logs</button>
    </a>
